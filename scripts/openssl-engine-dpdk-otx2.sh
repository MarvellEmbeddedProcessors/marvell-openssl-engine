...
cptpf="$(lspci -d 177d:a0fd | cut -d ' ' -f 1)"
cptdrv="$(lspci -k -d 177d:a0fd | grep "cpt" | grep -oE '[^ ]+$')"
echo $cptpf > /sys/bus/pci/drivers/$cptdrv/unbind

echo $cptpf > /sys/bus/pci/drivers/$cptdrv/bind
export OTX2_BUS="$(lspci -d 177d:a0fd | cut -d ':' -f 2 | cut -d ':' -f 1)"
